...
NONESSENTIAL_KEY_ITEMS = {
    KeyItemReward('#item.TwinHarp')        : RewardSlot.rydias_mom_item, #'twinharp_slot', 
...
def apply(env):
    ...
    while not found_valid_assignment and attempts < MAX_RANDOMIZATION_ATTEMPTS:
        ...
        if env.options.flags.has('key_items_vanilla'):
            ...
            rewards_assignment[RewardSlot.baron_castle_item] = EmptyReward() # (ItemReward('#item.') if env.options.flags.has('_in_key_items') else EmptyReward())
            used_keyitems = set()
